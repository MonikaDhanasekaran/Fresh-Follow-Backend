...
router.get("/searchContact/:key", async (req, res) => {
...
router.get('/getOne/:dataID', (req, res) => {
    try {
        contact.findOne({ id: req.params.dataID }, (err, data) => {
            if (err) {
...
router.post("/create", auth.authenticateUser, async (req, res) => {
    try {
        ...
        const newData = new contact(payload);
        await newData.save((err, data) => {
            if (err) {
                return res.status(400).send({ msg: "Error while adding data" });
            }
            res.status(201).send({ dataId: data._id, msg: "Message sent successfully" });
        });
    } catch (err) {
        res.status(500).send({ msg: "Internal Server Error" });
        console.log(err);
...
router.put("/update/:dataID", auth.authenticateUser, (req, res) => {
    try {
        contact.findByIdAndUpdate({ _id: req.params.dataID }, { $set: req.body }, (err, data) => {
            if (err) {
...
router.delete("/delete/:dataID", auth.authenticateUser, (req, res) => {
    try {
        contact.deleteOne({ _id: req.params.dataID }, (err, data) => {
            if (err) {
